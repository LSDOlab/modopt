...
class BFGS(ScipyOptimizer):
    def declare_options(self):
        ...
        self.options.declare('maxiter', default = None, types=(int, type(None)))
        self.options.declare('disp', default = True, types=bool)
        self.options.declare('eps', default = 1.4901161193847656e-08, types=float)
        self.options.declare('finite_diff_rel_step', default = None, types=(type(None), np.ndarray))
    ...
    def solve(self):
        ...
        result = minimize(obj, x0, args=(),  method=method, jac=grad, 
        hess=None, hessp=None, bounds=None, constraints=None, 
        tol=None, callback=callback, 
        options={'gtol' : gtol, 'norm' : norm, 'eps' : eps, 'maxiter': maxiter, 
