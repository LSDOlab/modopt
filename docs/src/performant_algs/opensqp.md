...
* - `hvp_stepsize`
  - *float* (`1e-6`)
  - Stepsize for computing the Hessian-vector products if \
    using a first-order finite-difference approximation.
* - `hvp_lmult_sign`
  - *int* (`-1`)
  - Indicates whether the user-supplied Lagrangian function (and its derivatives) \
    in the problem object adds or subtracts *lmult.T @ c* with the objective. \
    This is needed to accommodate the differences in formulation of the \ 
    Lagrangian in different problems. \
    If `-1`, *L = f - lmult * c*. \
    If  `1`, *L = f + lmult * c*.
* - `hvp_init_itr`
  - *int* (`10`)
...
* - `hvp_batch_size`
  ...
  - How many HVPs to compute and subsequent in-place BFGS updates to apply \
    within a single major iteration.
* - `hvp_dir`
  - *str* (`'GradKrylov'`)
  - Indicates directions along which to compute subsequent HVPs \
    after the first one is computed along the most recent step. \
    If `'StepHist'`, compute subsequent HVPs along the most recent steps. \
    If `'StepKrylov'`, use Krylov vectors generated by the Hessian and the most recent step. \
    If `'GradKrylov'`, use Krylov vectors of the Hessian and the current Lagrangian gradient.
* - `readable_outputs`
  - *list* (`[]`)
