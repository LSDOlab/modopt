'''Starship 2D trajectory optimization with Jax'''
...
def get_problem(nt): # 42 statements excluding comments, returns, grad, and nc computation.
    ...
    def jax_obj(v):
        ...
        # Cost function
        cost = jnp.sum(u[0]**2) + jnp.sum(u[1]**2) + 2*jnp.sum(x[5]**2)
    ...
    def jax_con(v):
        ...
        f = f.at[0, :].set(x[1, :-1])
        f = f.at[1, :].set(-thrust * jnp.sin(beta + theta) / m)
        f = f.at[2, :].set(x[3, :-1])
        f = f.at[3, :].set(thrust * jnp.cos(beta + theta) / m - g)
        f = f.at[4, :].set(x[5, :-1])
        f = f.at[5, :].set(-0.5 * L * thrust * jnp.sin(beta) / I)
    ...
    return JaxProblem(x0=np.ones(nt*8), nc=nc, jax_obj=jax_obj, jax_con=jax_con,
                      name=f'Starship {nt} timesteps Jax',
