'''Starship 2D trajectory optimization with CasADi'''
...
def get_problem(nt): # 42 statements excluding comments, and returns.
    ...
    def ca_obj(v):
        x = ca.reshape(v[:nt*6], (nt, 6)) # casadi is column-major
        u = ca.reshape(v[nt*6:], (nt, 2))

        # Cost function
        cost = ca.sum1(u[:, 0]**2) + ca.sum1(u[:, 1]**2) + 2*ca.sum1(x[:, 5]**2)
    ...
    def ca_con(v):
        ...
        f = ca.MX.zeros((nt-1, 6))

        thrust = max_thrust * u[:-1, 0] # thrust magnitude (N)
        theta  = x[:-1, 4]              # rocket angle (rad)
        beta   = u[:-1, 1]              # thrust angle / gimbal (rad)

        # Dynamics: xdot = f(x,u) = [xdot, xdotdot, ydot, ydotdot, thetadot, thetadotdot]
        f[:, 0] = x[:-1, 1]
        f[:, 1] = -thrust * ca.sin(beta + theta) / m    
        f[:, 2] = x[:-1, 3]
        f[:, 3] =  thrust * ca.cos(beta + theta) / m - g
        f[:, 4] = x[:-1, 5]
        f[:, 5] = -0.5 * L * thrust * ca.sin(beta) / I
    ...
    return CasadiProblem(x0=np.ones(nt*8), ca_obj=ca_obj, ca_con=ca_con,
                         name=f'Starship {nt} timesteps CasADi',
...
# print(np.linalg.norm(prob._compute_objective_gradient(np.arange(nt*8))))
# print(np.linalg.norm(prob._compute_constraint_jacobian(np.arange(nt*8))))
# exit()
